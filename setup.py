...
root = False
str = input('Do you have root privileges? [y/n] ').lower()
...
compMode = 0
str = input('Do you want to compile as a DelPhi-Module, as Stand-Alone or as Python module? [lib/exe/py] ').lower()
...
if (plat == 'darwin' and root==True):
        ...
        str = input('Do you want to continue? [y/n] ').lower()
        if (str == 'y'):
                pass
        else:
                quit()
        ...
        if (v[0]=='10' and v[1]=='6'):
                ...
                str = input('Do you want to install packets? [y/n] ').lower()
                if (str == 'y'):
        ...
        if (('suse' in distro) or ('opensuse' in distro)):
                print("Detected SUSE Linux distribution, installing required packages: boost, gmp, mpfr")
                print("")
                str = input('Do you want to install packets? [y/n] ').lower()
                if (str=='y'):
                        subprocess.call('yast -i boost boost-devel gmp gmp-devel mpfr mpfr-devel swig',shell=True)
        
        elif (('ubuntu' in distro) or ('debian' in distro)):
                print("Detected Debian based Linux distribution, installing required packages: boost, gmp, mpfr, g++") 
                str = input('Do you want to install packets? [y/n] ').lower()
                if (str=='y'):
                        ...
                        subprocess.call('apt-get -y install swig',shell=True)
                ...
                if (str=='y'):
                        ...
                        subprocess.call('yum install swig',shell=True)
...
if(os.path.isfile('./cmake-2.8.12.2/bin/cmake')==False):
  print("")
  print("Extracting cmake...")
  print("")
  subprocess.call('tar xvzf cmake-2.8.12.2.tar.gz',shell=True)

  print("")
  print("Compiling cmake...")
  print("")
  
  subprocess.call('cd cmake-2.8.12.2; ./bootstrap; make; make install',shell=True) 

# if cgal not found download and compile
if(os.path.isdir('./'+cgalDir)==False):
  print("")
  print("Extracting CGAL...")
  print("")
  ...
  if (mode32==0):
   build_cgal = 'cd '+cgalDir+' \n rm -f CMakeCache.txt \n sync \n ../cmake-2.8.12.2/bin/cmake . -DWITH_examples=false -DWITH_CGAL_Qt4=false -DWITH_CGAL_Qt3=false -DWITH_CGAL_ImageIO=false > cmake_cgal.txt \n sync \n make clean \n make > make_cgal.txt \n sync'
  else:
    # forcing 32 bits mode
    build_cgal = 'cd '+cgalDir+' \n rm -f CMakeCache.txt \n sync \n ../cmake-2.8.12.2/bin/cmake . -DWITH_examples=false -DWITH_CGAL_Qt4=false -DWITH_CGAL_Qt3=false -DWITH_CGAL_ImageIO=false -DCMAKE_CXX_FLAGS="-arch i386" > cmake_cgal.txt \n sync \n make clean \n make > make_cgal.txt \n sync'
...
if (mode32==0):    
        build_ns = 'cd '+where+' \n rm -fr * \n ../cmake-2.8.12.2/bin/cmake .. -DCGAL_DIR=%s/'%(pwd)+cgalDir+' > cmake_ns.txt \n make clean \n make > make_ns.txt'
else:
        # forcing 32 bits mode
        build_ns = 'cd '+where+' \n rm -fr * \n ../cmake-2.8.12.2/bin/cmake .. -DCGAL_DIR=%s/'%(pwd)+cgalDir+' -DCMAKE_CXX_FLAGS="-arch i386" > cmake_ns.txt \n make clean \n make > make_ns.txt'
...
if (compMode==0):  
  ...
  if (root==True):  
    ...
    subprocess.call('cd build_python \n python3 setupInstallation.py install > log.txt',shell=True)
